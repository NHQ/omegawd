...
tuner = {
	...
	init: function(){		
		...
		this.following = [];
		this.noRepeat = [];
		this.socket = io.connect('http://citizenmission.com/spfdr');
      ...
      console.log('hallo');
      tuner.socket.emit('data', {command: 'zrevrangebyscore', $et :['tags:analytics', '+inf', 0]})
    });
		this.socket.on('data', this.analytics);
    $('a').live('click', function(e){
      if($(this).hasClass('tag')){
        $('#hud')
          .empty()
          .prepend($('<h3>'+this.id+'</h3>'))
          .append($('<p id=score> Total hits</p>'))
          .append($('<p id=totallinks style=display:inline-inline> unique links<br />from </p><p style=display:inline-inline id=totalhosts> hosts</p>'))
          .append($('<a href=# class=zrevrangebyscore id='+this.id+':analytics>Show Hosts</a><br />'))
          .append($('<a href=# class=zrevrangebyscore id='+this.id+':links>Show Links</a>'))
          tuner.socket.emit('data', {command: 'zcard',$et: [this.id+':analytics'], target: 'totalhosts'})
          tuner.socket.emit('data', {command: 'zcard',$et: [this.id+':links'], target: 'totallinks'})
          tuner.socket.emit('data', {command: 'zscore', $et: ['tags:analytics', this.id], target: 'score'})
      }
      else
       tuner.socket.emit('data', {command: $(this).attr('class'), $et: [this.id, '+inf', 0], null, ''})
    })
	},
  analytics: function(data){
    if(data[2]){
      $('#'+data[2]).html(data[1]+$('#'+data[2]).html())
    }
    else{
      var tags = _.map(data[1], function(e){return '<a href=# class=tag id='+e+'>'+e.toLowerCase()+'</a>'}).join('<br />')
      $('#posts').empty().append($('<p>'+tags+'</p>'))
    }
  },
	select: function(){
		$('#posts').isotope({
